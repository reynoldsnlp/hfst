...
print_usage()
{
    ...
    fprintf(
        message_out,
        "Composition options:\n"
        "  -x, --xerox-composition=VALUE Whether flag diacritics are treated "
        "as ordinary\n"
        "                                symbols in composition (default is "
        "false).\n"
    ...
    fprintf(message_out, "VALUE can be one of the following: [true|false], "
                         "[yes|no] or [ON|OFF],\n");
...
compose_streams(HfstInputStream &firststream, HfstInputStream &secondstream)
{
    ...
    while (continueReading)
    {
        ...
        //  stream
        // if ((continueReading && secondstream.is_good()) || not
        // continueReading)
        //   {
        //     delete second;
        //     second=0;
