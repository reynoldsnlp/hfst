...
static bool force_ol = false; // accept also ol transducers when -R is not
...
print_usage()
{
    ...
    fprintf(message_out,
            ...
            "Perform transducer lookup (apply). Lookup is done from right to "
            "left,\n"
    ...
    fprintf(
        ...
        "possible\n"
        "  -f, --force-ol                   Force lookup of optimized lookup "
        "transducers (slow)\n");
    fprintf(message_out, "\n");
    ...
    /*    fprintf(message_out,
                ...
                "  Support VARIABLE 'print-space' for optimized lookup
       format\n");*/
    fprintf(message_out,
            /*"\n"*/
            "Known bugs:\n"
            "  * 'quote-special' quotes spaces that come from 'print-space'\n"
            "  * optimized lookup transducers are unidirectional and only "
            "support lookdown,\n"
...
is_valid_flag_diacritic_path(StringVector arcs)
{
    ...
    if (!res)
    {
        ...
        for (StringVector::const_iterator s = arcs.begin(); s != arcs.end();
             ++s)
        {
...
void
lookup_fd_and_print(HfstBasicTransducer &t, HfstOneLevelPaths &results,
