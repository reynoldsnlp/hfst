...
#include "inc/getopt-cases-error.h"
    ...
    else
    {
        ...
        infiles
            = static_cast<FILE **>(malloc(sizeof(FILE *) * (argc - optind)));
...
extend_matcher_with_options()
{
    ...
    if (match_word)
    {
        ...
        HfstTransducer wordBounded
            = nonWordCharLeft.concatenate(*matcher).concatenate(
                nonWordCharRight);
...
print_match_transducer(const HfstTransducer &path)
{
    ...
    for (vector<pair<string, string> >::const_iterator s
         = p.begin()->second.begin();
         s != p.begin()->second.end(); ++s)
    {
