...
print_usage()
{
    ...
    fprintf(message_out,
            "Other options:\n"
            "  -C, --check-negative-epsilon-cycles  Issue a warning if there "
            "are epsilon cycles\n"
            "                                       with a negative weight in "
            "the transducer\n"
    ...
    fprintf(message_out,
            ...
            "Space in transition symbols must be escaped as '@_SPACE_@' when "
            "using\n"
            "att format."
            "\n");
    fprintf(message_out, "\n");
    ...
    while (!feof(inputfile))
    {
        ...
        if (read_prolog_format)
        {
            ...
            if (output_format == hfst::XFSM_TYPE)
            {
                ...
                {
                    HfstTransducer *t
                        = HfstTransducer::prolog_file_to_xfsm_transducer(
                            inputfilename);
                    outstream << *t;
                    delete (t);
                    outstream.flush();
            ...
            try
            {
                HfstBasicTransducer fsm
                    = HfstBasicTransducer::read_in_prolog_format(inputfile,
                                                                 linecount);

                if (check_negative_epsilon_cycles)
                {
                    verbose_printf("Checking if the transducer has epsilon "
                    ...
                    {
                        verbose_printf("No epsilon cycles with a negative "
                                       "weight detected...\n");
                    }
                }
