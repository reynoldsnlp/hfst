...
void
set_output_type(ImplementationType type)
{
...
print_usage()
{
    ...
    fprintf(message_out,
            ...
            "  -b, --use-backend-format          Write result in "
            "implementation format, without any HFST wrappers\n"
            "  -S, --sfst                        Write output in (HFST's) "
            "SFST implementation\n"
            "  -F, --foma                        Write output in (HFST's) "
            "foma implementation\n"
            "  -x, --xfsm                        Write output in native xfsm "
            "format\n"
            "  -t, --openfst-tropical            Write output in (HFST's) "
            "tropical weight (OpenFST) implementation\n"
            "  -l, --openfst-log                 Write output in (HFST's) log "
            "weight (OpenFST) implementation\n"
            "  -O, --optimized-lookup-unweighted Write output in the HFST "
            "optimized-lookup implementation\n"
            "  -w, --optimized-lookup-weighted   Write output in "
            "optimized-lookup (weighted) implementation\n"
            "  -Q  --quick                       When converting to "
            "optimized-lookup, don't try hard to compress\n");
    fprintf(message_out, "\n");
    print_common_unary_program_parameter_instructions(message_out);
    fprintf(message_out,
            "FMT must be name of a format usable by libhfst, i.e. one of the "
            "following:\n"
            "{ foma, openfst-tropical, openfst-log, sfst, xfsm\n"
            "  optimized-lookup-weighted, optimized-lookup-unweighted }.\n"
            "Note that xfsm format is always written in native format without "
            "HFST wrappers.\n");
    fprintf(message_out, "\n");
    print_report_bugs();
    fprintf(message_out, "\n");
...
{
    if (instream.get_type() == hfst::FOMA_TYPE
...
main(int argc, char **argv)
{
    ...
    auto outstream
        = (outfile != stdout)
              ? std::make_unique<HfstOutputStream>(outfilename, output_type,
                                                   hfst_format)
