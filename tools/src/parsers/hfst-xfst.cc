...
print_usage()
{
    ...
    fprintf(
        ...
        "  -f, --format=FMT           Write result using FMT as backend "
        "format\n"
        ...
        "  -R, --restricted-mode      Allow read and write operations only in "
        "current\n"
        "                             directory, do not allow system calls\n"
        //          "  -k, --no-console         Do not output directly to
        ...
        "openfst-log, sfst }.\n"
        "Readline library, if enabled when configuring, is used for input by "
        "default.\n"
        ...
        "{both}.\n"
#ifdef _MSC_VER
        "If input file is not specified with -F, input is read interactively "
        "via the\n"
        "console, i.e. line by line from the user. If you redirect input from "
        "a file,\n"
        "use --pipe-mode=input. Output is by default printed to the console. "
        "If you\n"
...
parse_options(int argc, char **argv)
{
    ...
    while (true)
    {
        ...
        int c = getopt_long(argc, argv,
                            HFST_GETOPT_COMMON_SHORT "f:F:e:E:l:p::rwkR",
...
int
parse_file(const char *filename, hfst::xfst::XfstCompiler &comp)
{
...
void
insert_zeroes(char *array, unsigned int number)
{
...
static bool
expression_continues(std::string &expr)
{
    size_t s = expr.size();
...
main(int argc, char **argv)
{
    ...
    try
    {
        ...
        {
            verbose_printf(
                "Executing xfst command '%s' given on command line\n",
                cmd.c_str());
...
#ifdef WINDOWS
                    if (!pipe_output)
                        hfst::hfst_fprintf_console(
                            stderr, "expression '%s' could not be parsed\n",
                            expression.c_str());
                    else
#endif
                        fprintf(stderr,
        ...
        else
        {
            ...
            std::string expression
                = ""; // the whole expression (possibly several lines)
    ...
    {
        fprintf(stderr, "ERROR: trying to load automata in different "
                        "formats (OpenFst, foma) is not supported in XFST "
                        "scripts\n");
        return EXIT_FAILURE;
    }
    return EXIT_SUCCESS;
