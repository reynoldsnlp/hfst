...
#if HAVE_OPENFST
static hfst::ImplementationType compilation_format
...
print_usage()
{
    ...
    fprintf(message_out, "String and format options:\n"
                         ...
                         "      --cosine-distances    When compiling Like() "
                         "operations, include cosine distance info\n");
    fprintf(message_out, "\n");
    ...
    fprintf(message_out,
            "Examples:\n"
            "  echo \"Define TOP  UppercaseAlpha Alpha* LC({professor}) "
            "EndTag(ProfName);\" | %s \n"
...
process_stream(HfstOutputStream &outstream)
{
    ...
    if (definitions.count("TOP") == 1)
    {
        ...
        std::map<std::string, std::string> properties
            = definitions["TOP"]->get_properties();
            ...
            if (it->first.find("UNCOMPOSE") == string::npos)
            {
                harmonized_tmp = hfst::implementations::ConversionFunctions::
                    hfst_basic_transducer_to_hfst_ol(
                        intermediate_tmp,
                        true,             // weighted
                        "empty_alphabet", // empty alphabet in RTNs, they'll
                                          // use the main one
                        &harmonizer);     // harmonize with this
            }
            else
