...
print_usage()
{
    ...
    fprintf(
        ...
        "  -T, --to-transducer=TFILE    replace with transducer read from "
        "TFILE\n"
        "  -F, --from-file=LABELFILE    read replacements from LABELFILE\n"
        "  -R, --in-order               keep the order of the replacements\n"
        "                               (with -F)\n"
        "Input options:\n"
        "  -C, --do-not-convert         require that transducers in TFILE and "
        "INFILE\n"
        "                               have the same type\n"
        "Transient optimisation schemes:\n"
        "  -9, --compose                compose substitutions when "
        "possible\n");
    fprintf(message_out, "\n");
...
do_substitute(HfstBasicTransducer &trans, size_t transducer_n)
{
    if (from_pair && to_pair)
    {
        if (transducer_n < 2)
        {
            verbose_printf("Substituting pair %s:%s with pair %s:%s...\n",
                           from_pair->first.c_str(), from_pair->second.c_str(),
                           to_pair->first.c_str(), to_pair->second.c_str());
        }
        else
        {
            verbose_printf("Substituting pair %s:%s with pair %s:%s...\n",
    ...
    else if (from_label && to_label)
    {
        ...
        {
            verbose_printf(
                "Substituting label %s with label %s... " SIZE_T_SPECIFIER
                "\n",
    ...
    else if (from_pair && to_transducer)
    {
        ...
        {
            verbose_printf("Substituting pair %s:%s with transducer "
                           "%s... " SIZE_T_SPECIFIER "\n",
                           from_pair->first.c_str(), from_pair->second.c_str(),
                           to_name, transducer_n);
        }
    ...
    else if (from_label && to_transducer)
    {
        ...
        {
            verbose_printf("Substituting id. label %s with transducer "
                           "%s... " SIZE_T_SPECIFIER "\n",
                           from_label, to_name, transducer_n);
        }
...
do_substitute(HfstTransducer &trans, size_t transducer_n)
{
    ...
    else if (from_label && to_label)
    {
        if (compose)
        {
            ...
            HfstTransducer substitution(from_label, to_label,
                                        trans.get_type());
...
perform_delayed(HfstTransducer &trans)
{
    ...
    HfstTransducer sigmaMinusSubs = HfstTransducer(
        hfst::internal_identity, hfst::internal_identity, trans.get_type());
...
process_stream(HfstInputStream &instream)
{
    ...
    if (to_transducer_filename != NULL)
    {
        ...
        if (to_transducer_type != instream_type)
        {
            if (allow_transducer_conversion)
            {
                ...
                std::string warnstr(
                    "Transducer type mismatch in " + std::string(inputfilename)
                    + " and " + std::string(to_transducer_filename) + "; ");
                if (ct == 1)
                {
    ...
    while (instream.is_good())
    {
        ...
        {
            verbose_printf(
                "performing substitutions in %s... " SIZE_T_SPECIFIER "\n",
        ...
        if (from_file)
        {
            ...
            while (hfst_getline(&line, &len, from_file) != -1)
            {
                ...
                if (from_pair && to_pair)
                {
                    if (!in_order)
                    {
                        pair_substitution_map->operator[](*from_pair)
                            = *to_pair;
                ...
                else if (from_label && to_label)
                {
                    if (!in_order)
                    {
                        label_substitution_map->operator[](
        ...
        if (from_file)
        {
            char *composed_name = static_cast<char *>(
                malloc(sizeof(char)
                       ...
                       + 1));
            if (sprintf(composed_name, "substitute-from-%s", from_file_name)
                > 0)
            {
                hfst_set_name(trans, trans, composed_name);
                free(composed_name);
            }
            composed_name = static_cast<char *>(malloc(
                sizeof(char) * (strlen(from_file_name) + strlen("♲%s")) + 1));
            if (sprintf(composed_name, "♲%s", from_file_name) > 0)
            {
        ...
        else if (from_label && to_label)
        {
            char *composed_name = static_cast<char *>(
                malloc(sizeof(char)
                           * (strlen(from_label) + strlen(to_label)
                              + strlen("substitute-%s-with-%s"))
                       + 1));
            if (sprintf(composed_name, "substitute-%s-with-%s", from_label,
                        to_label)
                > 0)
            {
                hfst_set_name(trans, trans, composed_name);
            ...
            composed_name = static_cast<char *>(
                malloc(sizeof(char)
                           * (strlen(from_label) + strlen(to_label)
                              + strlen("%s ♲ %s"))
                       + 1));
            if (sprintf(composed_name, "%s ♲ %s", from_label, to_label) > 0)
            {
                hfst_set_formula(trans, trans, composed_name);
                free(composed_name);
            }
        ...
        {
            if (from_label
                == NULL) // make scan-build happy, this should not happen
                throw "Error: from_label has a NULL value.";

            char *composed_name = static_cast<char *>(malloc(
                sizeof(char)
                    * (strlen(from_label) + strlen(to_transducer_filename)
                       + strlen("substitute-%s-with-net-%s"))
                + 1));
            if (sprintf(composed_name, "substitute-%s-with-net-%s", from_label,
                        to_transducer_filename)
                > 0)
            {
                hfst_set_name(trans, trans, composed_name);
            ...
            composed_name = static_cast<char *>(malloc(
                sizeof(char)
                    * (strlen(from_label) + strlen(to_transducer_filename)
                       + strlen("%s ♲ %s"))
                + 1));
            if (sprintf(composed_name, "%s ♲ %s", from_label,
                        to_transducer_filename)
                > 0)
            {
                hfst_set_formula(trans, trans, composed_name);
