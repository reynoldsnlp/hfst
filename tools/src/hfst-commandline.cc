...
#endif
void
hfst_error_at_line(int status, int errnum, const char *filename,
                   unsigned int linenum, const char *fmt, ...)
{
    maybe_print_colour(stderr, COLOUR_BOLD);
    fprintf(stderr, "%s.%u: ", filename, linenum);
    maybe_print_colour(stderr, COLOUR_RED);
...
#endif
void
hfst_warning(int status, int errnum, const char *fmt, ...)
{
    maybe_print_colour(stderr, COLOUR_BOLD);
    fprintf(stderr, "%s: ", program_name);
...
int
get_compatible_fst_format(std::istream &, std::istream &)
{
    assert(false);
    return -1;
}
...
convert_transducers(hfst::HfstTransducer &first, hfst::HfstTransducer &second)
{
    ...
    else if (ct == -1)
    {
        ...
        HFST_THROW_MESSAGE(HfstFatalException,
                           "convert_transducers: conversion_type returned an "
                           "invalid integer");
...
bool
is_input_stream_in_ol_format(const hfst::HfstInputStream &is,
                             const char *program)
{
    ...
    {
        fprintf(stderr,
                "Error: %s cannot process transducers that are in optimized "
                "lookup format.\n",
                program);
        return true;
    }
...
hfst_strtoul(char *s, int base)
{
    ...
    else
    {
        hfst_error(EXIT_FAILURE, errno,
                   "%s is not a valid unsigned number string", s);
        return rv;
    }
}
...
hfst_strtol(char *s, int base)
{
    ...
    {
        hfst_error(EXIT_FAILURE, errno,
                   "%s is not a valid signed number string", s);
        return rv;
    }
}
...
hfst_parse_format_name(const char *s)
{
             ...
             || (strcasecmp(s, "ol") == 0))
    {
        rv = hfst::HFST_OLW_TYPE;
        hfst_warning(0, 0,
                     "Ambiguous format name %s, guessing "
...
hfst_read(int fd, void *buf, size_t count)
{
    ...
    {
        hfst_error(EXIT_FAILURE, 0,
                   "cannot read " SIZE_T_SPECIFIER " bytes in one read(2)",
                   count);
    }
    errno = 0;
...
hfst_setlocale()
{
    ...
    {
        hfst_error(EXIT_FAILURE, errno,
                   "Unable to set locale for character "
                   "settings");
    }
#else
    char *rv = NULL;
...
extend_options_getenv(int *argc, char ***argv)
{
    ...
    char **new_argv = static_cast<char **>(
        hfst_malloc(sizeof(char *) * (*argc + spaces + 1)));
    new_argv = static_cast<char **>(
...
should_colourise()
{
    ...
    else if (colour == COLOUR_ALWAYS)
    {
        return true;
    }
    else if (colour == COLOUR_NEVER)
    {
        return false;
    }
    else
...
void
maybe_print_colour(FILE *f, const char *colour)
{
    if (should_colourise())
    {
        fprintf(f, "%s", colour);
