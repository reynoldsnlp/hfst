 ...
 * HFST based tools. There is no common implementation for prototyped
 * functions, rather they must be defined on per program basis. For reference
...
bool is_input_stream_in_ol_format(const hfst::HfstInputStream &is,
                                  const char *program);

/* Common format into which transducers of types \a type1 and \a type2 will be
   converted, when convert_transducers will be called. Possible return values
   are:
...
int conversion_type(hfst::ImplementationType type1,
                    hfst::ImplementationType type2);

/* Convert transducers into common format, if needed. */
void convert_transducers(hfst::HfstTransducer &first,
                         hfst::HfstTransducer &second);
