...
Rule::Rule(const std::string &name, const RuleVector &v)
    : is_empty(true), name(unescape_name(name)), rule_transducer(TWOLC_UNKNOWN)
{
    rule_transducer.apply(&HfstTransducer::repeat_star);
    for (RuleVector::const_iterator it = v.begin(); it != v.end(); ++it)
    {
        if (!(*it)->empty())
        {
...
std::string
Rule::get_print_name(const std::string &s)
{
    std::string ss(s);
...
void
Rule::store(HfstOutputStream &out)
{
    if (is_empty)
    {
        return;
    ...
    rule_transducer.apply(&HfstTransducer::substitute, TWOLC_EPSILON,
                          HFST_EPSILON, true, true);
...
OtherSymbolTransducer
Rule::get_universal_language_with_diamonds(void)
{
    ...
    universal_with_diamonds.apply(&HfstTransducer::concatenate, diamond)
...
OtherSymbolTransducer
Rule::get_center(const std::string &input, const std::string &output)
{
    ...
    center.apply(&HfstTransducer::concatenate, diamond)
...
OtherSymbolTransducer
Rule::get_center(const SymbolPairVector &v)
{
    OtherSymbolTransducer unknown(TWOLC_UNKNOWN);
    unknown.apply(&HfstTransducer::repeat_star);
    OtherSymbolTransducer diamond(TWOLC_DIAMOND);
    ...
    for (SymbolPairVector::const_iterator it = v.begin(); it != v.end(); ++it)
    {
        ...
        center_pair_transducer.apply(&HfstTransducer::disjunct, pair);
    }
    OtherSymbolTransducer center(unknown);
    center.apply(&HfstTransducer::concatenate, diamond)
...
OtherSymbolTransducer
Rule::get_center(const OtherSymbolTransducer &restricted_center)
{
    OtherSymbolTransducer unknown(TWOLC_UNKNOWN);
    unknown.apply(&HfstTransducer::repeat_star);
    OtherSymbolTransducer diamond(TWOLC_DIAMOND);
