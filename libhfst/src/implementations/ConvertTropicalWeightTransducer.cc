...
static void
handle_symbol_tables(fst::StdVectorFst *t, HfstBasicTransducer *net,
...
static void
copy_alphabet(fst::StdVectorFst *t, HfstBasicTransducer *net)
{
    const fst::SymbolTable *inputsym = t->InputSymbols();
    const fst::SymbolTable *outputsym = t->OutputSymbols();
    ...
    fst::StdVectorFst *t, bool has_hfst_header)
{
    ...
    StringVector symbol_vector
        = TropicalWeightTransducer::get_symbol_vector(t);

    std::vector<unsigned int> harmonization_vector
        = HfstTropicalTransducerTransitionData::get_harmonization_vector(
            symbol_vector);
    ...
    for (fst::StateIterator<fst::StdVectorFst> siter(*t); !siter.Done();
         siter.Next())
    {
        ...
        for (fst::ArcIterator<fst::StdVectorFst> aiter(*t, s); !aiter.Done();
             aiter.Next())
        {
            ...
            if (arc.ilabel >= symbol_vector.size())
            {
                ...
                oss << "FATAL ERROR: input number " << arc.ilabel
                    << " not in symbol_vector" << std::endl;
            ...
            {
                std::ostringstream oss;
                oss << "FATAL ERROR: output number " << arc.olabel
                    << " not in symbol_vector" << std::endl;
                HFST_THROW_MESSAGE(HfstFatalException, oss.str());
                // exit(1);
            }
            ...
            net->add_transition(
                origin,
                HfstBasicTransition(
                    target, harmonization_vector[arc.ilabel],
                    harmonization_vector[arc.olabel], arc.weight.Value(),
    ...
    const HfstBasicTransducer *net)
{
         ...
         it != net->end(); it++)
    {
        ...
        for (HfstBasicTransitions::const_iterator tr_it = it->begin();
             tr_it != it->end(); tr_it++)
        {
            // Copy the transition
            ...
            t->AddArc(state_vector[source_state],
                      fst::StdArc(in, out, tr_it->get_weight(),
    ...
    {
        t->SetFinal(state_vector[it->first], it->second);
    }
