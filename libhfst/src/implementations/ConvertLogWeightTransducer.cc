...
#ifndef _MSC_VER
#if HAVE_OPENFST_LOG // if HAVE_LEAN_OPENFST_LOG is requested, these are not
                     // defined elsewhere (?)
namespace fst
{
...
/* Create an HfstBasicTransducer equivalent to an OpenFst log weight
   transducer \a t. */
HfstBasicTransducer *
ConversionFunctions::log_ofst_to_hfst_basic_transducer(LogFst *t,
    ...
    for (fst::StateIterator<LogFst> siter(*t); !siter.Done(); siter.Next())
    {
        ...
        if (s == initial_state)
        {
                 ...
                 aiter.Next())
            {
                ...
                net->add_transition(
                    origin, HfstBasicTransition(target, istring, ostring,
                                                arc.weight.Value()));
            }
...
StateId
ConversionFunctions::hfst_state_to_state_id(
    HfstState s, std::map<HfstState, StateId> &state_map, LogFst *t)
{
...
LogFst *
ConversionFunctions::hfst_basic_transducer_to_log_ofst(
    const HfstBasicTransducer *net)
{
         ...
         it != net->end(); it++)
    {
             ...
             tr_it != it->end(); tr_it++)
        {
            // Copy the transition
            t->AddArc(
                hfst_state_to_state_id(source_state, state_map, t),
                fst::LogArc(st.AddSymbol(tr_it->get_input_symbol()),
                            st.AddSymbol(tr_it->get_output_symbol()),
                            tr_it->get_weight(),
                            hfst_state_to_state_id(tr_it->get_target_state(),
                                                   state_map, t)));
        }
    }
