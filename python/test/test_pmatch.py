...
for type in [hfst_dev.ImplementationType.SFST_TYPE, hfst_dev.ImplementationType.TROPICAL_OPENFST_TYPE, hfst_dev.ImplementationType.FOMA_TYPE]:
    if hfst_dev.HfstTransducer.is_implementation_type_available(type):
        ...
        cont = hfst_dev.PmatchContainer(defs)
        ...
        with open('streets.txt', 'r') as myfile:
            data=myfile.read()
            myfile.close()
        defs = hfst_dev.compile_pmatch_expression(data)
        cont = hfst_dev.PmatchContainer(defs)
        assert cont.match("Je marche seul dans l'avenue des Ternes.") == "Je marche seul dans l'<FrenchStreetName>avenue des Ternes</FrenchStreetName>."
        ...
        try:
            hfst_dev.compile_pmatch_file(nonexistent_file)
            assert False
        except IOError as e:
            pass
        ...
        if os.path.isfile('finnish-tokenizer.hfstol'):
            #istr = hfst_dev.HfstInputStream('finnish-tokenizer.hfstol')
            #defs = istr.read_all()
            #istr.close()
            #cont = hfst_dev.PmatchContainer(defs)
            cont = hfst_dev.PmatchContainer('finnish-tokenizer.hfstol')
        ...
        for expr in ["foobarbaz; ????", "", "efine CapWord UppercaseAlpha Alpha* :"]:
            print(expr)
            try:
                defs = hfst_dev.compile_pmatch_expression(expr)
                assert False
            except hfst_dev.exceptions.HfstException as e:
                pass
